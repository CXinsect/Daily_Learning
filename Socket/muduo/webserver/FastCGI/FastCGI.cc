...
int FastCGI::makeNameValueBody(char *name, int nameLen, char *value,
...
int FastCGI::sendStartRequestRecord() {
  ...
  beginRecord.header = makeHeader(FCGI_BEGIN_REQUEST, c->requestId_,
...
int FastCGI::sendParams(char *name, char *value) {
  ...
  makeNameValueBody(name, strlen(name), value, strlen(value), bodyBuff,
                    &bodyLen);
...
char *FastCGI::findStartHtml(char *p) {
  enum { S_NOPE, S_CR, S_CRLF, S_CRLFCR, S_CRLFCRLF } state = S_NOPE;
