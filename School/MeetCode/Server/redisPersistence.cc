...
string Persistence::saveBackGround() {
    ...
    if((pid = fork()) == 0) {
        ...
        if(!database_->getKeySpaceStringObject().size() &&
          !database_->getKeySpaceHashObject().size() &&
          !database_->getKeySpaceListObject().size()) {
               return Status::IOError("The DataBase is Empty").ToString();
        }
       char buf[1024] = {0};
       std::string tmp = getcwd(buf,sizeof(buf));
       assert(tmp.c_str() != NULL);
       ...
       std::cout << "tmp: The File Path: " << tmp << std::endl;
       //保存文件
       bool temp = rdbSave(tmp);
       exit(temp == true ? 0 : -1);
    } else {
        if(pid == -1) return Status::IOError("fork error").ToString();
